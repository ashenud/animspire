<?php
    ...
    switch ($status){
        ...
        case "delete_freelancer":
            ...
            $result = $adminObj->deleteFreelancer($freelancer_id);
    
            if ($result == 1) {

                $msgSuccess = "Freelancer Successfully Deleted !";
                $msgSuccess = base64_encode($msgSuccess);
                
                ?>
                    <script>window.location = "../view/user/system_admin/admin-freelancer-management.php?msgSuccess=<?php echo $msgSuccess; ?>" </script>  
                <?php
            }
            else {
                $msg = "Freelancer not Deleted!";
                $msg = base64_encode($msg);
                
                ?>
                    <script>window.location = "../view/user/system_admin/admin-freelancer-management.php?msg=<?php echo $msg; ?>" </script>  
                <?php
            }
            
        break;

        case "activate_freelancer":
    
            $freelancer_id = $_REQUEST["freelancer_id"];
            
            $result = $adminObj->activateFreelancer($freelancer_id);
    
            if ($result == 1) {

                $msgSuccess = "Freelancer Successfully Activated !";
                $msgSuccess = base64_encode($msgSuccess);
